...
export const Route = createFileRoute("/contact")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Have questions about scholarships, universities or courses? Contact the EduGuide team — we'd love to hear from you." },
      ...
      <section className="container-px mx-auto max-w-6xl py-16 grid lg:grid-cols-5 gap-10">
        <div className="lg:col-span-2 space-y-5">
          {[
            ...
            <div key={i.title} className="p-6 rounded-2xl bg-soft border border-border flex items-start gap-4">
              <span className="grid place-items-center w-11 h-11 rounded-xl bg-primary/10 text-primary"><i.icon className="w-5 h-5" /></span>
              <div>
        ...
        <form onSubmit={(e) => e.preventDefault()} className="lg:col-span-3 p-8 rounded-2xl bg-card border border-border shadow-card space-y-5">
...
function Field({ label, ...rest }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div>
      ...
      <input {...rest} className="w-full h-12 px-4 rounded-xl bg-background border border-border focus:outline-none focus:ring-2 focus:ring-primary" />
    </div>
  );
}
