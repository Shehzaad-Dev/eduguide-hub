...
export const Route = createFileRoute("/scholarships")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse fully funded scholarships in the USA, UK, Europe and worldwide. Find scholarships for undergraduate, master and PhD students." },
      { property: "og:title", content: "Scholarships — EduGuide" },
      { property: "og:description", content: "Hand-picked fully funded scholarships for international students." },
    ...
    <PageShell>
      ...
      <section className="container-px mx-auto max-w-7xl py-16">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {SCHOLARSHIPS.map((s) => (
            <article key={s.name} className="p-7 rounded-2xl bg-card border border-border hover:shadow-card transition flex flex-col">
              ...
              <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                <div><dt className="text-muted-foreground text-xs">Country</dt><dd className="font-medium">{s.country}</dd></div>
                <div><dt className="text-muted-foreground text-xs">Level</dt><dd className="font-medium">{s.level}</dd></div>
              </dl>
